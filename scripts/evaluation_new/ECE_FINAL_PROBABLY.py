...
def compute_metrics_path(mode, model, std, n_pert):
    assert mode in ['naive','consistency'], 'The metrics paths mode should be \'naive\' or \'consistency\''
    model_name = model.split('/')[-1]
    return f'Results/{model_name}_std_{std:.2f}_npert_{n_pert}_{mode}'.replace('.', '_')+'.csv'
...
def get_forecasts_cc(test_data_input, predictions, pipeline, scales): #predictions should be of shape (num_series, pred_length, num_samples)
    forecasts = []
    predicted_ts = []
    ...
    assert isinstance(predictions, torch.Tensor), 'The predictions array is neither a tensor nor a numpy ndarray'

    for i,samples in enumerate(predictions):
        scale = scales[i]
        predicted_ts.append(pipeline.tokenizer.output_transform(samples.to(scale.device), scale))
...
def compute_metrics(forecasts_naive, forecasts_cons, test_data, batch_size=5000):
    metrics_naive = (
        evaluate_forecasts(forecasts_naive, test_data=test_data,
    ...
    metrics_cons = (
        evaluate_forecasts(forecasts_cons, test_data=test_data,
            metrics=[
                MASE(),
...
def main(
    ...
    for config in backtest_configs:
        ...
        predicted_ts_naive, forecasts_naive = get_forecasts_cc(test_data.input, naive_sample_tokens, pipeline, scales)
        predicted_ts_cons, forecasts_cons = get_forecasts_cc(test_data.input, cons_sample_tokens, pipeline, scales)
        ...
        ECE = MulticlassCalibrationError(num_classes = naive_probs.shape[-1],
                                        n_bins = n_bins)
        ...
        metrics_naive, metrics_cons = compute_metrics(forecasts_naive, forecasts_cons, test_data)
