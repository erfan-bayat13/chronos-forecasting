...
def main(
    ...
    use_cyclic_lr: bool = False,  # Enable Cyclic Learning Rate if True
    base_lr: float = 1e-5,        # Minimum learning rate for CLR
    max_lr: float = 1e-3,         # Maximum learning rate for CLR
    step_size_up: int = 2000,     # Steps for increasing LR in CLR
    mode: str = "triangular",     # CLR mode (triangular, exp_range, etc.)
):
    if tf32 and not (
    ...
    # Apply Cyclic Learning Rate if enabled
    if use_cyclic_lr:
        scheduler = CyclicLR(
            optimizer,
            base_lr=base_lr,
        ...
        log_on_main("Using Cyclic Learning Rate scheduler", logger)
    else:
    ...
    # Pass the optimizer and scheduler to Trainer
    trainer = Trainer(
        model=model,
        args=training_args,
        train_dataset=shuffled_train_dataset,
        optimizers=(optimizer, scheduler)  # <-- Added optimizer & scheduler
    )
